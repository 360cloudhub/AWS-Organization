...
resource "aws_organizations_organizational_unit" "ou" {
  ...
  name      = var.ou_name
  parent_id = length(var.ou_parent_id) > 0 ? var.ou_parent_id : data.aws_organizations_organization.org.roots[0].id
}
...
resource "aws_organizations_account" "account" {
  ...
  parent_id = length(var.account_parent_id) > 0 ? var.account_parent_id : data.aws_organizations_organization.org.roots[0].id
